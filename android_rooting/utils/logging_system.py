...
class SecurityLogger:
    ...
    def log_audit_event(self, event: AuditEvent):
        ...
        if event.severity.value >= LogLevel.SECURITY.value:
            self.security_logger.warning("SECURITY_EVENT", extra={'audit_data': event_dict})
            
        # Log to compliance for all events
        self.compliance_logger.info("COMPLIANCE_EVENT", extra={'audit_data': event_dict})
...
class RootingLogger:
        ...
        for log_file in self.log_dir.glob("*.log"):
            try:
                if log_file.stat().st_size > max_size:
                    self.warning(f"Log file {log_file.name} is large: {log_file.stat().st_size / 1024 / 1024:.1f}MB")
            except Exception:
                pass
    ...
    def audit(self, event_type: EventType, description: str, 
              details: Optional[Dict] = None, severity: LogLevel = LogLevel.AUDIT):
        """Log audit event"""
    ...
    def log_performance(self, operation: str, duration: float, details: Optional[Dict] = None):
        ...
        self.performance_data = [p for p in self.performance_data if p['timestamp'] > cutoff]
...
def main():
    ...
    if args.action == 'test':
        ...
        # Test audit logging
        logger.audit(EventType.ROOT_ATTEMPT, "Test root attempt", 
