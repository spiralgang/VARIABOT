...
class KaliIntegration:
    ...
    def _detect_kali_environment(self) -> KaliEnvironment:
        ...
        try:
            ...
            # Check for NetHunter
            if os.path.exists("/usr/share/kali-nethunter") or os.path.exists(
                "/system/etc/init.d/99nethunter"
            ):
                env.nethunter_installed = True

            # Check for LLM support
            if (
                os.path.exists("/opt/llm")
        ...
        self.logger.info(
            f"Kali environment: {env.is_kali}, NetHunter: {env.nethunter_installed}"
        )
        return env
    ...
    def _start_nethunter_services(self):
        ...
        for service in services:
            try:
                if self.nethunter_config and os.path.exists(
                    self.nethunter_config.chroot_path
                ):
        ...
        try:
            ...
            if "ollama" in self.tools:
                ...
                ollama_start = subprocess.run(
                    ["ollama", "serve"], capture_output=True, timeout=5
                )
...
"""
    ...
    def _query_local_llm(self, prompt: str) -> str:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"prompt": prompt, "max_tokens": self.llm_config["max_tokens"]},
                timeout=120,
            )
    ...
    def generate_comprehensive_report(
        self, scan_data: Dict, pentest_data: Dict, llm_analysis: Dict
    ) -> str:
...
<body>
    ...
    def _generate_recommendations_section(
        self, pentest_data: Dict, llm_analysis: Dict
    ) -> str:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Kali Linux Integration for Android Security Testing"
    )
    parser.add_argument(
        "action",
        choices=[
    ...
    try:
        if args.action == "env-info":
            env_info = {
                ...
                "nethunter_config": (
                    kali.nethunter_config.__dict__ if kali.nethunter_config else None
                ),
            ...
            report_path = kali.generate_comprehensive_report(
                scan_data, pentest_data, llm_analysis
            )
